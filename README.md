...
基于 LangGraph 的 **Supervisor 模式** 多智能体，总控路由到 **闲聊 Agent** 与 **知识库 Agent**；支持前端上传文档 → MinerU 解析 → 对比/自定义 → 确认后写入 Elasticsearch；知识库侧支持 **高频 QA 精准匹配**、**Text2SQL**、**RAG（动态权重 BM25+HNSW+BGE3+重排）**。
...
- **知识库 Agent**：三种机制  
  ...
  3. **RAG**：全文检索与向量检索按**动态权重**混合（默认 3:7，按查询与语料的词汇覆盖率自动调整）；检索前支持按 `doc_id / category / doc_path` 先 **filter 缩小范围**再检索；**切片策略**支持多种切块大小验证、父块约 **150 字重叠**；**检索评估**（匹配度、命中位置、无关信息比例等），低于阈值则重检（最多 3 次）；答案**展示依据来源**并返回依赖的切片。

- **文档流程**：上传 → MinerU 解析（占位使用父子切片策略）→ 返回解析结果 → 前端对比/自定义 → 确认后上传到 Elasticsearch。
...
- Python 3.10+
- 复制 `.env.example` 为 `.env`，填写 DeepSeek API Key、Elasticsearch 等（勿使用 OpenAI Key 调用 OpenAI 模型）。
...
- 接口摘要：
  ...
  - `POST /doc/confirm_upload`：body 中传 `task_id`，可选传自定义 `chunks`，确认后写入 Elasticsearch。
  - `POST /chat`：对话，body `{ "message": "用户输入", "thread_id": "可选", "retrieval_filter": "可选（doc_id/category/doc_path/doc_path_prefix）" }`，返回 `reply`、`thread_id`。
...
- **Elasticsearch**：需先启动 ES，配置 `ES_URI`、`ES_INDEX`（维度与 BGE-M3 一致，默认 1024）；应用会自动创建带 `dense_vector` 映射的索引。
...
- `config/settings.py`：`rag_chunk_sizes`（可验证的切块大小）、`rag_parent_overlap`（默认 150）、`rag_bm25_ratio` / `rag_vector_ratio`（默认 0.3 : 0.7，可关闭动态权重）、`rag_dynamic_weight_enabled` / `rag_bm25_ratio_min` / `rag_bm25_ratio_max`（动态调整区间）、`rag_min_match_score`（默认 0.3）、`rag_max_retrieve_attempts`（默认 3）。
...
- RAG：扩展 BM25 语料、Elasticsearch 与 `retrieve_with_validation` 逻辑。
