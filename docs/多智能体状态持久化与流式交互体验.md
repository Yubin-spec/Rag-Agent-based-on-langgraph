...
- **父子图共享 state**：`knowledge` 被实现为一个子图；LangGraph 上为 **`knowledge_qa_rag` + `knowledge_text2sql`** 两节点：`knowledge_qa_rag` 内按 **Text2SQL 预判 → 高频 QA → Text2SQL/RAG 路由** 与 `KnowledgeEngine` 对齐，RAG 在同节点执行，需执行 SQL 时经边进入 `knowledge_text2sql`（`src/kb/text2sql.py`），Text2SQL 无结果时用 `kb_rag_only` 回到 `knowledge_qa_rag` 只跑 RAG。父图调用子图时不通过 Redis 之类的“共享键”通信，而是让子图**接收并返回同一份 AgentState**；子图节点通过读写 state 字段（如 `messages/pending_sql/next`）把结果“回传”给父图。
