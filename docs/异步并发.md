...
| **知识库 aquery** | 分步：to_thread(规则 Text2SQL 短路 / QA / 检索) + await RAG 生成 | 仅 CPU/同步 IO 用线程；RAG 答案生成用 `_rag_chain.ainvoke`。 |
| **知识库 aquery_stream** | to_thread(规则 Text2SQL 短路 / QA / 检索) + await _generate_grounded_answer_async | 同上，流式下 RAG 生成也不占线程。 |
