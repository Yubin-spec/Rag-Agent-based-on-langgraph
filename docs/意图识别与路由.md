...
```text
进入 knowledge_qa_rag 节点后（与 KnowledgeEngine 一致）：
  Text2SQL 规则强命中？ → 是 → 经边进入 knowledge_text2sql（跳过高频 QA）
  → 否 → 高频 QA 匹配命中？ → 是 → 直接结束
  → 否 → 二级路由（Text2SQL vs RAG，规则+LLM+低置信澄清）
            → Text2SQL：经边进 knowledge_text2sql（对应 src/kb/text2sql.py）→ 结束；未命中则 kb_rag_only 回 knowledge_qa_rag 只跑 RAG
            → RAG：仍在 knowledge_qa_rag 内检索+生成（含二次 RAG）→ 结束
```
...
- **规则短路（高性能）**：明显数据查询/删改意图 → **在 knowledge_qa_rag 内先短路**进 Text2SQL 分支（跳过高频 QA）；QA 未命中后的混合路由里，规则仍可倾向 Text2SQL 或 RAG。
