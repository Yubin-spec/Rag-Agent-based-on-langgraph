...
POST /chat/stream
  ...
  → 根据路由：
      ...
      - 知识库：engine.aquery_stream → 顺序与 query 一致（Text2SQL 预判 → QA → Text2SQL → RAG）；
                QA/Text2SQL 结果用 _yield_text_chunked 按段 yield；RAG 生成用 _generate_grounded_answer_stream → _rag_chain.astream 逐 token yield
