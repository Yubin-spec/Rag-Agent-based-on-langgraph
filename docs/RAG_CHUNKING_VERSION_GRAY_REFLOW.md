...
- `_get_table_spans(text)` 会先按“连续 table 行”（每行至少含 2 个 `|`）粗分出若干表区间；
- 当相邻两个表区间之间只有“空白行或页码标记行”（如 `第 N 页` / `Page N` / `p.N` / `-N-`），则会将这两个区间合并为同一张表的 span；
- 从而在 `_adjust_spans_for_tables()` 里避免“跨页续表被切成两段 span”，保证后续不会在续表中间截断。
