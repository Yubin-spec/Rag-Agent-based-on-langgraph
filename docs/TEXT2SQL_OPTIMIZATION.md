...
当前实现见 `src/kb/text2sql.py`：意图判断（规则）→ Schema 缓存 + 人工关联 → 生成 SELECT/写操作 → SQL 格式清洗 → 校验（仅 SELECT、语法、多表关联）→ 执行 → LLM 生成自然语言答案。以下为可选的进一步优化方向。
...
- **问题**：模型输出不稳定——可能包裹在 ` ```sql ``` ` 代码块中、混入中文标点（`，` `；` `（` `）`）、包含零宽字符/BOM 等乱码、前面加解释文字、或输出多条 SQL。
- **做法**：统一使用 `_sanitize_and_extract_sql(raw)` 处理所有 LLM 原始输出，替代原来分散在各处的简单正则。处理链路：
  1. 代码块围栏提取（` ```sql ... ``` `）
  2. 行内代码提取（`` `SELECT ...` ``）
  3. LLM 前缀废话剥离（"以下是生成的SQL："等）
  4. SQL 关键字定位（从混合文本中找到 SELECT/DELETE/UPDATE 起始位置）
  5. 乱码字符清除（零宽字符、BOM、控制字符）
  6. 中文标点转 ASCII（`，` → `,`、`（` → `(`、`）` → `)`、`；` → `;`）
  7. 多语句拆分取首条
  8. 尾部清理
- **位置**：`src/kb/text2sql.py` 中 `_sanitize_and_extract_sql()`，在 `query()` 和 `_query_write_confirm()` 中所有 LLM 输出提取点统一调用。
...
- **现状**：模型直接输出 SQL，经 `_sanitize_and_extract_sql` 清洗后使用。
...
| SQL 格式清洗     | `text2sql.py` 中 `_sanitize_and_extract_sql()`，统一处理代码块/乱码/中文标点/多语句 |
...
以上部分已在代码中实现（LIMIT 保护、执行错误重试、SQL 格式清洗），其余可按优先级逐步落地。
