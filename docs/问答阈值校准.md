...
当前高频匹配在知识链路里的执行顺序是（与 `KnowledgeEngine.query` / `aquery` 一致）：
...
3. **未命中回退**
   - 在 **KnowledgeEngine 单入口**下：高频 QA 不通过门控后**直接走 RAG**，不再在 QA 之后做一次「全量 Text2SQL」。
   - 在 **LangGraph 知识子图**（`knowledge_qa_rag` + `knowledge_text2sql` 两节点）下：QA 未命中时由混合路由决定去 Text2SQL 节点，或 **在 `knowledge_qa_rag` 节点内**直接跑 RAG；Text2SQL 无结果则 `kb_rag_only` 回到 `knowledge_qa_rag` 只跑 RAG。
...
如果任意条件不通过，则高频 QA 视为“不匹配”：`KnowledgeEngine` 单入口下继续走 **RAG**；LangGraph 知识子图下由路由决定 **Text2SQL 节点或 RAG 节点**。
